...
import { Sparkles, Palette, Shirt, Check, ArrowRight, ArrowLeft } from 'lucide-react'
...
const STYLE_TYPES = [
    { value: 'casual', label: 'Casual', emoji: '👕', description: 'Relajado y cómodo' },
    { value: 'formal', label: 'Formal', emoji: '👔', description: 'Profesional y elegante' },
    { value: 'sporty', label: 'Deportivo', emoji: '🏃', description: 'Activo y dinámico' },
    { value: 'elegant', label: 'Elegante', emoji: '✨', description: 'Sofisticado y refinado' },
    { value: 'bohemian', label: 'Bohemio', emoji: '🌸', description: 'Artístico y libre' },
]
...
export default function OnboardingFlow({ onComplete }: OnboardingProps) {
    ...
    const steps = [
        { title: 'Bienvenido a UNIX', subtitle: 'Tu asistente personal de moda con IA' },
        { title: 'Identifica tu tono de piel', subtitle: 'Esto nos ayudará a recomendarte colores que te favorecen' },
        { title: 'Elige tu estilo', subtitle: 'Personaliza tus recomendaciones según tu gusto' },
        { title: '¡Listo!', subtitle: 'Generando tu perfil de moda personalizado...' },
    ]
                ...
                <div className="mb-8">
                    <div className="flex justify-between mb-2">
                        {steps.map((s, i) => (
                            <div
                                key={i}
                                className={`flex-1 h-1 rounded-full mx-1 transition-all duration-500 ${i <= step ? 'bg-gradient-to-r from-purple-500 to-pink-500' : 'bg-slate-800'
                                    }`}
                            />
                        ))}
                    </div>
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        {step === 0 && (
                            ...
                            >
                                ...
                                <p className="text-xl text-slate-300 mb-8">{steps[step].subtitle}</p>
                                <div className="space-y-4 text-left max-w-md mx-auto mb-8">
                                    {[
                                        { icon: Palette, text: 'Análisis de colorimetría personalizado' },
                        ...
                        {step === 1 && (
                            <motion.div
                                ...
                                exit={{ opacity: 0, x: -20 }}
                            >
                                <h2 className="text-3xl font-bold mb-2 text-white text-center">{steps[step].title}</h2>
                                <p className="text-slate-400 mb-8 text-center">{steps[step].subtitle}</p>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    {SKIN_TONES.map((tone) => (
                                        <motion.button
                                            ...
                                            className={`relative p-6 rounded-2xl border-2 transition-all ${skinTone === tone.value
                                        ...
                                        >
                                            ...
                                            <h3 className="font-bold text-white mb-1">{tone.label}</h3>
                                            <p className="text-sm text-slate-400 mb-3">{tone.description}</p>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                    {STYLE_TYPES.map((style) => (
                                        <motion.button
                                            ...
                                            className={`relative p-6 rounded-2xl border-2 transition-all ${styleType === style.value
                                        ...
                                        >
                                            ...
                                            <h3 className="font-bold text-white mb-1 text-sm">{style.label}</h3>
                        ...
                        {step === 3 && (
                            ...
                            >
                                ...
                                <h2 className="text-3xl font-bold mb-4 text-white">{steps[step].title}</h2>
